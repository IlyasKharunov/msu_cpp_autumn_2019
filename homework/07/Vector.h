...
public:
	...
	Vector(Vector<T, Alloc> && a)
		: data_(a.data_), size_(a.size_), capacity_(a.capacity_)
		{
			a.data_ = nullptr;
			a.size_ = 0;
			a.capacity_ = 0;
		}
	Vector(const Vector<T, Alloc> & a)
		: data_(alloc_.allocate(a.capacity_)), size_(0), capacity_(a.capacity_)
	{
		for (; size_ < a.size_; size_++) alloc_.construct(data_ + size_, a[size_]);
